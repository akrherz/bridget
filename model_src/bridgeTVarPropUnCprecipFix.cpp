...
/* This program is designed to calculate a concrete bridge's surface temperature.  
   ...
   //anticipated publication (as of June 2004):
   //Greenfield, T. M., 2004: Bridge frost prediction by heat and mass transfer methods.
   // M.S. thesis, Dept. of Geological and Atmospheric Science, Iowa State University
...
float evaporation(float, float, float, float, float, float);
...
int main (int argc, const char * argv[])
{
   ...
   while( mm5file>>year>>junk>>month>>junk>>day>>junk>>hour>>junk>>junky>>minutesf>>
          ntempA>>nqa>>nwind>>nSWradiationIn>>nLWradiationIn>>nprecipRate )
   {  //cout<<nprecipRate<<endl;
...
//cout<<" New"<<nprecipRate<<" current"<<cprecipRate<<" old"<<oprecipRate;
//cout<<linecounter<<"  ";
            // quality checks
...
float d;
	    d = log(cqa*1000.0/(0.622*6.1078))/21.875;
	    float Tf;
	    Tf = (-7.66*d +273.0)/(1.0-d);
	    //cout<<Td<<"  "<<Tf<< endl;
...
{  /* MM5 calculates precip rate in centimeters per hour, and the bridge will not 
   ...
   heatFlux = 0;
   precip = precip / 6000.0 ; // centimeters per hour to meters per minute
   ...
   //cout<<precipDepth<<"  "<<precip<<"new  ";
   
   if ( precipDepth > 0.0011 ) // derived experimentally  
   {
      precipDepth = 0.0011;
   }
   if (Tw <= 273.16 && bridgeTemp <= 273.16 ) // snow on bridge.  
   {
   ...
   //this is the equilibrium temperature between precip accumulation and the bridge
   ...
   {
      evapRate = evaporation ( hm, M, R, bridgeTemp, Patm, airTemp);
      condition = 0;//dry
      if(totalDepth > 0.0)
      {
         condition = 5;//wet
   ...
   if ( Tw >= 273.16 && bridgeTemp <= 273.16 ) // freezing/condensation/evaporation
   {
      ...
      {
	 heatFlux = kbridge * ( 273.16 - Te ) / (thickness);//q' = k dT/dx
         freezeRate =  heatFlux / Lf;//the amount that will freeze with the amount
	 // of excess energy available between the equilibrium temperature and 273.16

	 if (freezeRate/waterDensity * 60 <= (totalDepth - frozen))
	 {
            frozen = freezeRate/waterDensity * 60 + frozen;
	    condition = 4; //freezing
	 }
	 else// if possible freezing exceeds whats available to freeze (all frozen)
	 {
	    heatFlux = (totalDepth - frozen) / 60.0 * waterDensity;
	    frozen = totalDepth;
            condition = 2;//icy/snowy
         }
	 Tw = 273.16;
	    
         if ( frozen > precipDepth )//takes care of straggling precip
         {
            condition = 2;//icy/snowy
            if (frostDepth > 0.0 )
            {
   ...
   else if ( Tw <= 273.16 && bridgeTemp <= 273.16 ) // snow/frost. sublimation, no melting
   {
      // frost accumulation/evaporation possible
      // this version does allow frost formation over existing precipitation accumulation, but
      // it will not be labeled as "frost," just additional accumulation of snow/ice.

      evapRate = evaporation ( hm, M, R, bridgeTemp, Patm, airTemp);
      if (  precipDepth ==0 )
      {
         //frost will form if evapRate is negative -- water flux toward the bridge surface
         // depth after a minute of accumulation/evap.  Density is a tenth of water.
         frostDepth += ( -600 * evapRate / waterDensity );
	 condition = 1;//frosty
         if (frostDepth <= 0  )
         {
            frostDepth =0.0;
	    ...
	    condition = 0; //dry
         ...
         Tw = bridgeTemp;
	 //if(evapRate < 0.0)
	 //{ cout<<evapRate<<" "<<precipDepth<<"  "<<frostDepth<<endl;}

      }

      // only 0.025 in water (~.25 in. of snow) allowed because of plowing
      //Snow density assumed 1/10 of water.
      //cout<<Tw<<" "<<bridgeTemp<<" "<<Te<<endl;
      Tw = bridgeTemp = Te;
      if (precipDepth >0 && frostDepth == 0)
      {
         condition = 2;//icy/snowy
   ...
   else if ( Tw <= 273.16 && bridgeTemp > 273.16 ) // melting possible
   {
      ...
      {
         heatFlux = kbridge * ( 273.16 - Te ) / (thickness);//q' = k dT/dx
         freezeRate =  heatFlux / Lf;//the amount that will melt with the amount
	 // of excess energy available between the equilibrium temperature and 273.16

	 if (freezeRate/waterDensity * 60 <= frozen)
         {
            frozen = freezeRate/waterDensity * 60 + frozen;
            condition = 3;//melting
	 }
         else// if possible melting exceeds whats available to melt
	 {
	    heatFlux = frozen / 60.0 * waterDensity;
	    frozen = 0;
            condition = 5;//wet
         }
	 frostDepth = freezeRate/waterDensity * 60 + frostDepth;
         ...
         if ( frozen < 0 ||  frostDepth <0)// just in case frostDepth or frozen goes -
         {
            Tw = Te;
            bridgeTemp = Te;
            frostDepth =0;
   ...
   //decreases or increases the total depth based on evaporation rate.
   // doesn't alter melting or freezing values
   if (condition != 3 && condition != 4)
   {
      if ( totalDepth > (evapRate * 60 / waterDensity) )
      {
	 ...
	 if(frostDepth > 0.0)
	 {
	    precipDepth = 0.0;
         ...
         heatFlux = -1 * evapRate * Lv;
         if (condition == 2 || condition == 1)
         {
	    heatFlux = -1 * evapRate * (Lv + Lf);
         }
	 //cout<<evapRate<<"evap "<<totalDepth<<"  td bigger "<<heatFlux<<"  "<<(evapRate * 60 / waterDensity)<<endl;
      }


      else // when evapRate exceeds total depth
      {
         heatFlux = -1 * totalDepth * waterDensity * Lv / 60.0;
         if (condition == 2 || condition == 1)
         {
            heatFlux = -1 * totalDepth * waterDensity * (Lv + Lf) / 60.0;
         }
         totalDepth = 0;
         precipDepth = 0;
         Mw = 0;
         condition = 0;//dry
	 //cout<<heatFlux<<" evap exceeds "<<i<<endl;
      }
   }
   //corrects if totalDepth or precipDepth go negative for some reason
   if ( totalDepth < 0 || precipDepth < 0 || frostDepth < 0)
   {
      heatFlux = 0.0;
      totalDepth = 0.0;
      precipDepth = 0.0;
...
//cout<<"precipdepth"<<precipDepth<<"   ";
   return heatFlux;
}  

float evaporation (float hm, float M, float R, float bridgeTemp, float Patm, float airTemp)
{
   //this is a function to calculate the evaporation rate over water or ice, depending on
   // bridge temperature.
   
   static float Psat;// pressure of water vapor at saturation
   static float evapRate;//evaporation rate kg/s/m2
   
   if (bridgeTemp > 273.16)
   {
   ...
   evapRate = hm * M / R * ( Psat / bridgeTemp - Patm / airTemp);
   // kg/sec per unit area
...
{   /* n is used for node definition, it gives the vertical position of the node.  
    ...
    */
    ...
    {  
       ...
       {
          cout << "  heating/cooling rate too high.  check inputs  "<<
