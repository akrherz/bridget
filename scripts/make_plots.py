...
PVARS = {
    "bdeckt": {
        "title": "Bridge Deck Temperature [F]",
        "units": "F",
        "levels": np.arange(-30, 61, 5),
    },
    "frostd": {
        "title": "Frost Depth [mm]",
        "units": "mm",
        "levels": np.arange(0.0001, 0.05, 0.005),
    },
    "ifrost": {
        "title": "Frost Present (yes/no)",
        "units": "",
        "levels": [0, 1, 2],
    },
}
...
def make_plots(nc):
    ...
    for i, tm in enumerate(nc.variables["time"][:]):
        ...
        for pvar in PVARS:
            m = MapPlot(
                title="ISUMM5/Bridget Modelled %s" % (PVARS[pvar]["title"],),
            ...
            pqstr = (
                "plot c %s model/frost/bridget/%02i/%s_%02i_f%03i.png bogus png"
                % (sts.strftime("%Y%m%d%H%M"), sts.hour, pvar, sts.hour, fhour)
            )
